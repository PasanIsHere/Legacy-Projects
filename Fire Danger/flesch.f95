...
program main
    ...
    read(*,*) fname
    wordLen = 0 
    prev1Char = 'z' !char to hold previous character of words
    prev2Char = 'z' !char to hold pre-previous charcter of words
    numSentences = 0
    numWords = 0
        ...
        if(EOF == 0) then
            if (index(" .,;?!:", curChar) /= 0 .or. ichar(curChar) == 10 ) then
                ...
                if(wordFlag .eqv. .false.) then
                    ...
                    prev1Char = 'z'
                    prev2Char ='z'
                    !if the word has less than 3 or less then it only has 1 syllable
                    if(wordLen <= 3) then
                        wordSyllables = 1
                    end if
